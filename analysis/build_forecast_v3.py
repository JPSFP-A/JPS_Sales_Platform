...
lines = [
    ...
    ("1) RT40 and RT50 contain real 'Postpaid'/'Zero'/'Zero-Postpaid' rows by parish, present in every month except June 2026.", ITALIC_RED),
    ("   customer_count here is small (single digits to low 20s per parish) - not clearly a mass-residential pattern.", ITALIC_RED),
    ("   Could be real small aggregated postpaid commercial accounts, or a smaller-scale mistagging - unconfirmed.", ITALIC_RED),
    ...
    ("2) KAM column on RT40/50/60-ST/70 comes from jps_kam (jps_ac -> kam). Only accounts with an assigned KAM show a name; the rest show '-'.", ITALIC_RED),
    ("", None),
